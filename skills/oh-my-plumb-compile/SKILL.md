...
- A source listed as MCP server instructions is not a file: its text is quoted in the list the hook gave you. Its rules are about that server's tools, so they are tool-call rules (step 3, question 1) scoped to the glob the list names, such as `["mcp__postgres__*"]`.
